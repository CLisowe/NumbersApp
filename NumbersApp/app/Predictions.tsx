...
import WeatehrWidget from "@/components/WeatherWidget";
...
export default function LiveInventoryScreen() {
  ...
  const fetchProducts = async () => {
    ...
    const { data, error } = await supabase
      .from("products")
      .select("id,name,quantity,is_favorite")
  ...
  const createProduct = async (p: { name: string; quantity: number }) => {
    ...
    setProducts((prev) => [
      ...prev,
      { id: data.id, name: data.name, quantity: data.quantity, is_favorite: data.is_favorite },
    ]);
  };
